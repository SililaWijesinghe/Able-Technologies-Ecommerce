      ...
      <div className="md:mt-[25px] grid grid-cols-2 sm:grid-cols-3 lg:flex lg:flex-nowrap justify-between gap-4 md:gap-6 pt-4">
        {displayCards.map((card, idx) => {
          ...
          <Link to={targetUrl} key={idx} className="flex-1 min-w-[140px] block">
            ...
            >
              <div className="w-20 h-20 md:w-24 md:h-24 rounded-lg mb-3 md:mb-4 relative flex items-center justify-center">
                 {card.icon_url ? (
                   <img src={card.icon_url} alt={card.name || card.title} className="w-full h-full object-cover drop-shadow-md" />
                 ) : (
                   <span className="text-gray-400 text-[10px] md:text-xs">Image</span>
                 )}
                 <div className="absolute top-1.5 right-1.5 bg-white p-1 md:p-1.5 rounded-full shadow-sm text-blue-900">
