...
export default function Categories() {
  ...
  // Form states
  const [addForm, setAddForm] = useState({ name: '', slug: '', description: '', icon_url: '' });
  const [editForm, setEditForm] = useState({ id: '', name: '', slug: '', description: '', icon_url: '' });
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch products for counts & potential auto-import
      const { data: prodData, error: prodErr } = await supabase.from('products').select('category, category_id');
      
      // Auto-import fallback
      if (currentCats.length === 0 && prodData && prodData.length > 0) {
        const distinctCats = [...new Set(prodData.map(p => p.category).filter(Boolean))];
        if (distinctCats.length > 0) {
          const newCats = distinctCats.map(name => ({
            name: String(name),
            slug: String(name).toLowerCase().replace(/[\\s\\W-]+/g, '-'),
            description: \`Auto-imported category\`,
            icon_url: ''
          }));
          const { data: inserted, error: insertErr } = await supabase.from('categories').insert(newCats).select();
          if (!insertErr && inserted) {
            currentCats = inserted;
            toast({ title: "Auto-Import Successful", description: \`Imported \${inserted.length} categories from products.\`, variant: "success" });
          }
        }
      }
      ...
      prodData?.forEach(p => {
        const matched = currentCats.find(c => c.id === p.category_id || c.name === p.category || c.slug === p.category);
        if (matched) {
  ...
  const handleAdd = async (closeModal: () => void) => {
    ...
    try {
      ...
      const { error } = await supabase.from('categories').insert([addForm]);
      if (error) throw error;

      toast({ title: "Success", description: "Category created successfully!", variant: "success" });
      setAddForm({ name: '', slug: '', description: '', icon_url: '' });
      closeModal();
      fetchData();
    } catch (err: any) {
                ...
                <>
                  ...
                  <Modal.Body className="space-y-5 py-6">
                    <Input 
                      ...
                      variant="bordered"
                      value={addForm.name}
                      onValueChange={(val) => setAddForm({...addForm, name: val, slug: val.toLowerCase().replace(/[\\s\\W-]+/g, '-')})}
                    ...
                    <Input 
                      ...
                      variant="bordered"
                      value={addForm.slug}
                      onValueChange={(val) => setAddForm({...addForm, slug: val})}
                      classNames={{ inputWrapper: "border-white/20 bg-white/5", label: "text-white/70", input: "text-white font-mono text-sm" }}
                    />
                    <div className="flex flex-col gap-1.5">
                      <label className="text-sm text-white/70 font-medium px-1">Description</label>
                      <textarea 
                        className="w-full bg-white/5 border border-white/20 rounded-xl p-3 text-white placeholder:text-white/30 focus:outline-none focus:border-blue-400 focus:ring-1 focus:ring-blue-400 transition-all text-sm"
                        rows={3}
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-white/10 text-xs uppercase tracking-wider text-white/50">
                ...
                <th className="px-6 py-4 font-semibold">Slug & Desc</th>
            ...
            <tbody className="divide-y divide-white/5">
              {loading ? (
                <tr>
                  <td colSpan={5} className="px-6 py-16 text-center text-white/50">
                ...
                categories.map((cat) => (
                  <tr key={cat.id} className="hover:bg-white/5 transition-colors group">
                    ...
                    <td className="px-6 py-4 max-w-[200px]">
                      <div className="text-white/70 text-sm truncate">{cat.description || 'No description'}</div>
                      <div className="text-white/40 text-xs font-mono mt-1">/{cat.slug}</div>
                    </td>
                    <td className="px-6 py-4">
                              ...
                              <>
                                ...
                                <Modal.Body className="space-y-5 py-6">
                                  ...
                                  <Input 
                                    ...
                                    variant="bordered"
                                    value={editForm.slug}
                                    onValueChange={(val) => setEditForm({...editForm, slug: val})}
                                    classNames={{ inputWrapper: "border-white/20 bg-white/5", label: "text-white/70", input: "text-white font-mono text-sm" }}
                                  />
                                  <div className="flex flex-col gap-1.5">
                                    <label className="text-sm text-white/70 font-medium px-1">Description</label>
